// Direct port of ThreeWireProtocol.js
...
// CRC16 calculation (exact port from original CRC16.js)
var crcTable = [
...
function CalculateCrc(value: number[]): number[] {
  let crc = 65535
...
// Direct port of original ThreeWireProtocol.js
export class ThreeWireProtocol {
  Protocol = AdapterProtocolService.getInstance()

  async SendKeySignature(): Promise<void> {
    // For now, we don't support SendKeySignatureAndReadyReq feature
  ...
  async InitSession(): Promise<void> {
    const deviceInfo = this.Protocol.getDeviceInfo?.() || { name: 'KFDTool' }
    
    await this.clearSessionBuffers()
    await this.sendReadyRequest()
    await this.waitForReadyResponse(deviceInfo)
  }

  /**
   * Clear session buffers before initialization
   */
  private async clearSessionBuffers(): Promise<void> {
    if (this.Protocol && 'clearBuffers' in this.Protocol && typeof this.Protocol.clearBuffers === 'function') {
      this.Protocol.clearBuffers()
    }
  }

  /**
   * Send ready request to radio
   */
  private async sendReadyRequest(): Promise<void> {
    await this.Protocol.SendData([OPCODE_READY_REQ])
  }

  /**
   * Wait for and validate ready response
   */
  private async waitForReadyResponse(deviceInfo: any): Promise<void> {
    await this.waitForDevice(deviceInfo)
    
    const response = await this.Protocol.GetByte(TIMEOUT_STD, false)
    if (response !== OPCODE_READY_GENERAL_MODE) {
      throw new Error(`InitSession: unexpected opcode 0x${response?.toString(16)}`)
    }
  }

  /**
   * End radio session with graceful cleanup
   */
  async EndSession(): Promise<void> {
    const deviceInfo = this.Protocol.getDeviceInfo?.() || { name: 'KFDTool' }
    
    try {
      await this.sendTransferDone(deviceInfo)
      await this.sendDisconnect(deviceInfo)
    } catch (error) {
      console.error("EndSession failed with error:", error)
      this.clearProtocolBuffers()
      // Don't throw - allow recovery in next session
    }
  }

  /**
   * Send transfer done command and wait for response
   */
  private async sendTransferDone(deviceInfo: any): Promise<void> {
    console.log("kfd: transfer done")
    await this.Protocol.SendData([OPCODE_TRANSFER_DONE])
    
    console.log("mr: transfer done")
    await this.waitForDevice(deviceInfo)
    
    const response = await this.Protocol.GetByte(TIMEOUT_STD, true)
  ...
  /**
   * Send disconnect command and wait for acknowledgment
   */
  private async sendDisconnect(deviceInfo: any): Promise<void> {
    console.log("kfd: disconnect")
    await this.Protocol.SendData([OPCODE_DISCONNECT])
    
    console.log("mr: disconnect ack")
    await this.waitForDevice(deviceInfo)
    
    const response = await this.Protocol.GetByte(TIMEOUT_STD, true)
    if (response !== OPCODE_DISCONNECT_ACK) {
      console.error("mr: unexpected opcode in disconnect phase")
      throw new Error(`Unexpected disconnect response: 0x${response?.toString(16)}`)
    }
  }

  /**
   * Wait appropriate time based on device type
   */
  private async waitForDevice(deviceInfo: any): Promise<void> {
    const deviceName = deviceInfo.name
    if (deviceName === "KFDTool" || deviceName === "KFD-AVR" || deviceName === "KFDMicro") {
      await new Promise(resolve => setTimeout(resolve, 150))
    }
  }

  /**
   * Clear protocol buffers for error recovery
   */
  private clearProtocolBuffers(): void {
    if (this.Protocol && 'clearBuffers' in this.Protocol && typeof this.Protocol.clearBuffers === 'function') {
      this.Protocol.clearBuffers()
    }
  }
  ...
  async CheckRadioHealth(): Promise<boolean> {
    try {
      // Quick health check - just try to initialize and end a session
      await this.SendKeySignature()
      await this.InitSession()
      await this.EndSession()
      return true
    } catch (error) {
      console.warn("Radio health check failed:", error)
      return false
    }
  }

  async CreateKmmFrame(kmm: number[]): Promise<number[]> {
    // create body
    let body: number[] = []
    ...
    frame.push(crc[0]) // crc low byte
    frame.push(crc[1]) // crc high byte

    return frame
  }

  async ParseKmmFrame(): Promise<number[]> {
    let temp: number
    let length = 0
    ...
    if (expectedCrc[1] != crc[1]) {
      console.error("mr: crc high byte mismatch")
    }
    
    return kmm
  }

  async SendKmm(inKmm: number[]): Promise<void> {
    console.log("TWP.SendKmm inKmm", inKmm.map(b => b.toString(16).padStart(2, '0')).join("-"))
    if (inKmm.length > 512) {
      console.error("kmm exceeds max size")
    }
    
    let txFrame = await this.CreateKmmFrame(inKmm)
    console.log("TWP.SendKmm txFrame", txFrame.map(b => b.toString(16).padStart(2, '0')).join("-"))
    await this.Protocol.SendData(txFrame)
  }

  async PerformKmmTransfer(inKmm: number[]): Promise<number[]> {
    console.log("TWP.PerformKmmTransfer inKmm", inKmm.map(b => b.toString(16).padStart(2, '0')).join("-"))
    // send kmm frame
    ...
    console.log("MR -> KFD KMM FRAME:", rxFrame.map(b => b.toString(16).padStart(2, '0')).join("-"))

    return rxFrame
  }

  async MrRunProducer(): Promise<void> {
    // NOT IMPLEMENTED
  }
}
